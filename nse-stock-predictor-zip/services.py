...
import features as feature_layer
...
# Raw OHLCV caching now lives in loader.py (disk-cache + Parquet store),
# so this in-process cache is only used for the (expensive, model-training)
...
def get_stock_dataframe(
    ...
    Data flow (Analysis Orchestrator): loader.get_historical() checks the
    disk cache, then the Parquet/DuckDB store, and only talks to yfinance
    for what's actually missing/stale — see loader.py. This function then
    layers on indicators/patterns/features exactly as before.

    Raises
    ------
    TickerNotFoundError
        Bad symbol, delisted, or no data returned anywhere (cache, store,
        or provider).
    InsufficientDataError
        Data returned but too short to compute indicators reliably.
    DependencyMissingError
        yfinance isn't installed and nothing is cached/stored either.
    """
    resolved = resolve_ticker(ticker)

    try:
        df = data_layer.get_historical(resolved, period=period, interval=interval, force_refresh=not use_cache)
    except data_layer.DataUnavailableError as exc:
        if "yfinance is not installed" in str(exc) or "not installed" in str(exc):
    ...
    # Additive: EMA(9/21/50), Supertrend, VWAP - doesn't touch the columns
    # add_technical_indicators() already produced (SMA/RSI/MACD/BB/ATR).
    try:
        df = feature_layer.add_features(df, prefer_pandas_ta=False)
    except Exception:  # noqa: BLE001 - feature layer is additive/best-effort
        logger.exception("features.add_features failed for %s; continuing without the extra set", resolved)
...
def get_stock_payload(
    ...
    return {
        ...
        "latest": {
            ...
            "supertrend": _safe_float(latest.get("Supertrend")),
            "supertrend_direction": _safe_float(latest.get("Supertrend_Direction")),
