...
# --------------------------------------------------------------------------- #
# Both are OFF-by-default-behavior-preserving:
#   - Auth only activates once API_KEYS is set in the environment (see
#     config.py / .env.example) - with no keys configured, every request
#     is allowed through, same as before this layer existed.
#   - Rate limiting defaults ON but generous (60 req/min); set
#     RATE_LIMIT_ENABLED=false to disable it entirely.
# /api/health, /, /docs, /redoc, /openapi.json are always exempt so
# uptime checks and the interactive docs work with no key.
...
async def auth_and_rate_limit(request: Request, call_next):
    ...
    try:
        auth.check_api_key(api_key)
        auth.enforce_rate_limit(request, api_key)
    except HTTPException as exc:
        return JSONResponse(
            status_code=exc.status_code,
            content={"error": "unauthorized" if exc.status_code == 401 else "rate_limited", "detail": exc.detail},
...
@app.exception_handler(data_layer.DataUnavailableError)
async def data_unavailable_handler(request: Request, exc: data_layer.DataUnavailableError):
    # Covers "market closed / no live quote yet", "provider unreachable and
    # nothing cached/stored", etc. — a clear, actionable message rather than
    # a raw stack trace.
...
async def health():
    return {
        "status": "ok",
        "service": "nse-stock-prediction-api",
        "auth_enabled": config.AUTH_ENABLED,
        "rate_limit_enabled": config.RATE_LIMIT_ENABLED,
        "stored_tickers": len(storage.list_stored_tickers()),
    }
...
# GET /api/live/{ticker}
# --------------------------------------------------------------------------- #
@app.get("/api/live/{ticker}", tags=["stock"])
async def get_live(ticker: str):
    """
    Best-effort *current* price snapshot — separate from
    /api/stock/{ticker}'s historical (Parquet-backed) data on purpose:
    live quotes are never persisted to the Parquet store, only short-TTL
    cached (see loader.get_live_quote / config.LIVE_QUOTE_CACHE_TTL_SECONDS),
    since they represent an in-progress, not-yet-closed session.

    Returns 503 with a clear message (not a raw stack trace) if the
    market/provider has nothing to report right now (e.g. market closed
    and the provider returns no live tick, or the provider is unreachable).
    """
    resolved = services.resolve_ticker(ticker)
    try:
        return data_layer.get_live_quote(resolved)
    except data_layer.DataUnavailableError:
        raise
    except Exception as exc:  # noqa: BLE001
        logger.warning("get_live(%s) failed: %s", resolved, exc)
        raise data_layer.DataUnavailableError(
            f"Live quote unavailable for '{resolved}' right now (market may be closed, "
            f"or the data provider is temporarily unreachable): {exc}"
        ) from exc
